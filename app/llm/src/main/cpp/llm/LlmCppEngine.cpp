...
namespace lunacattus::llm {
    ...
    bool LlmCppEngine::IsContextFull() const {
        return chat_term_.current_position >= kDefaultContextSize - kOverflowHeadroom;
    }

    // ==================== 会话状态管理 ====================

    void LlmCppEngine::ResetSystemTerm(bool clear_kv_cache) {
        // 清除消息记录和位置追踪
    ...
    void LlmCppEngine::ResetUserTerm() {
        chat_term_.stop_generation_position = 0;
        chat_term_.cached_token_chars.clear();
        chat_term_.assistant_ss.str("");          // 清空 stringstream
    }

    ChatTerm &LlmCppEngine::chat_term() {
        return chat_term_;
    }

    // ==================== 提示词处理 ====================

    std::string LlmCppEngine::FormatPrompt(bool is_system_role, const char *prompt,
    ...
    std::vector<int>
    LlmCppEngine::TokenizePrompt(const std::string &prompt) {
    ...
    int LlmCppEngine::CheckTokensLength(bool is_system_role, std::vector<int> &tokens) {
    ...
    int
    LlmCppEngine::DecodeTokensInBatches(const std::vector<int> &tokens, bool compute_last_logit,
    ...
    void LlmCppEngine::ShiftContext() {
    ...
    int LlmCppEngine::SampleNextToken() {
    ...
    int LlmCppEngine::PopulateBatchAndDecode(int new_token_id) {
    ...
    bool LlmCppEngine::CheckTokenEndOfGenerate(int new_token_id) {
        // 判断是否为 EOS/EOT 结束标记
    ...
    std::string LlmCppEngine::ConvertTokenToText(int new_token_id) {
    ...
    std::string LlmCppEngine::AddChat(const std::string &role, const std::string &content,
                                      bool enable_thinking) {
